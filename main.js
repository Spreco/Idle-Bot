...
try {
    ...
    console.error('Nema settings fajla! (./config/settings.js)');
    console.error('Zatrazite pomoc na www.facebook.com/adnan.gold17');
    process.exit(1);
}
...
function processMessage(botid, senderid, message) {
    ...
    if (settings.botAdmins.indexOf(senderid.getSteamID64()) > -1) {
        logger.verbose('[' + bots[botid].name + '] Poruka pristigla od Administratora: ' + message);
        if (message.substr(0, 1) === '!') {
            var command = message.split(' ')[0].substring(1).toLowerCase();
            switch (command) {
                case 'komande':
                    bots[botid].bot.chatMessage(senderid, 'Dostupne komande: !komande, !info, !kartice, !idle <appid>,!stani, !refresh, !aktiviraj <kod>');
                    break;
                case 'info':
                    bots[botid].bot.chatMessage(senderid, 'GEternal Bot by Spreco');
                    break;
                case 'status':
                case 'stats':
                case 'drops':
                ...
                case 'kartice':
                    ...
                    Object.keys(bots).forEach(function(id) {
                        if (bots.hasOwnProperty(id)) {
                            ...
                            cards += bot_cards;
                            bots[botid].bot.chatMessage(senderid, '[' + bots[id].name + '] ' + bot_cards + ' kartica' + ' ostalo jos (' + Object.keys(bots[id].apps).length + ' igre' + ')' + (bots[id].idling ? ', Trenutno idlam: ' + bots[id].idling : '') + '!');
                        }
                    });
                    bots[botid].bot.chatMessage(senderid, cards + ' ostalo jos na ' + Object.keys(bots).length + ' botu' + '!');
                    break;
                case 'botidle':
                case 'idle':
                    var game = message.split(' ')[1];
                    idleGame(botid, game);
                    bots[botid].bot.chatMessage(senderid, 'Pocinjem idlat ' + game);
                    break;
                case 'farmidle':
                ...
                case 'stani':
                    stopIdle(botid, game);
                    bots[botid].bot.chatMessage(senderid, 'Prestajem da idlam!');
                    break;
                case 'botstart':
                case 'botrefresh':
                case 'refresh':
                    bots[botid].bot.chatMessage(senderid, 'Refreshujem bota!');
                    updateGames(botid, function(err) {
                        if (err) {
                            bots[botid].bot.chatMessage(senderid, 'Error! ' + err);
                        } else {
                            bots[botid].bot.chatMessage(senderid, 'Uspjesno!');
                        }
                    });
                    break;
                case 'farmrefresh':
                ...
                case 'feed':
                    ...
                    botRedeemKey(botid, code, function(err, botid, apps) {
                        if (err) {
                            bots[botid].bot.chatMessage(senderid, 'Nemoguce aktivirati key, neispravan: ' + err);
                        } else {
                            bots[botid].bot.chatMessage(senderid, 'Key uspjesno aktiviran! Nova igrica/ce: ' + Object.keys(apps).map(function(index) {
                                    return apps[index];
                                }).join(', ') + '!');
                        }
                    });
                ...
                case 'debug':
                    ...
                    break;
                default:
                    bots[botid].bot.chatMessage(senderid, 'Nepoznata komanda, pokusaj: !komande');
            }
        }
    } else {
    ...
    loadBadges(botid, 1, apps, function(err, apps) {
        ...
        if (Object.keys(apps).length > 0) {
            ...
            if (!bots[botid].idling || !apps.hasOwnProperty(bots[botid].idling)) {
                ...
                logger.verbose('[' + bots[botid].name + '] Igra promjenuta!');
                idleGame(botid, Object.keys(apps)[0]);
            } else {
                logger.verbose('[' + bots[botid].name + '] Igra nije promjenuta!');
            }
        } else {
