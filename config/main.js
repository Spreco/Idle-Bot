...
try {
    ...
    console.error('Za pomoc javi se www.facebook/adnan.sprecic');
    process.exit(1);
}
...
function processMessage(botid, senderid, message) {
    ...
    if (settings.botAdmins.indexOf(senderid.getSteamID64()) > -1) {
        ...
        if (message.substr(0, 1) === '!') {
            ...
            switch (command) {
                case 'komande':
                    bots[botid].bot.chatMessage(senderid, 'Dostupne komande: !komande, !info, !cards, !idle <appid>, !farmidle <appid>, !botstop, !refresh, !farmrefresh, !redeem <code>');
                    break;
                case 'info':
                    bots[botid].bot.chatMessage(senderid, 'Geternal Bot 2017');
                    break;
                case 'status':
                case 'stats':
                case 'drops':
                ...
                case 'cards':
                    ...
                    bots[botid].bot.chatMessage(senderid, cards + 'Ostalo jos ' + Object.keys(bots).length + ' botu'(bject.keys(bots).length === 1 ? '' : 's') + '!');
                    break;
                case 'botidle':
                case 'idle':
                ...
                case 'farmidle':
                    ...
                    farmIdle(all_game);
                    bots[botid].bot.chatMessage(senderid, 'Svi botovi sa ovim igrom pocinju idlati!: ' + all_game);
                    break;
                case 'botstop':
                    stopIdle(botid, game);
                    bots[botid].bot.chatMessage(senderid, 'Prestao sam idlati');
                    break;
                case 'botstart':
                case 'botrefresh':
                case 'refresh':
                    bots[botid].bot.chatMessage(senderid, 'Refresujem bota!');
                ...
                case 'feed':
                    ...
                    botRedeemKey(botid, code, function(err, botid, apps) {
                        if (err) {
                            ...
                            bots[botid].bot.chatMessage(senderid, 'Aktiviran kod! Nove igrica: ' + Object.keys(apps).map(function(index) {
                                    return apps[index];
                                }).join(', ') + '!');
                        }
                    });
                    break;
                case 'adminkomande':
                    bots[botid].bot.chatMessage(senderid, '!2fa');
                    break;
                case '2fa':
                    Object.keys(bots).forEach(function(id) {
